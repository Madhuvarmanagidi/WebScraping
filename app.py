...
def scrape_alphaminds(url, schema, website_name):
    ...
    try:
        ...
        for heading in soup.find_all(["h3", "h4"]):
            ...
            description = (" ".join(description_parts)).strip()[:300] if description_parts else "N/A"
...
class GoogleSheetLoader:
    ...
    def append_rows(self, rows, header=None):
        ...
        batch_values = []
        for r in rows:
            batch_values.append([r.get(h, "N/A") for h in header])
    ...
    for row in items:
        for k, v in list(row.items()):
            if "date" in k.lower():
                ...
                row[k] = ensure_field(v)

    header = page_config["schema"]
    loader.append_rows(items, header=header)
    print(f"  Inserted {len(items)} rows into sheet for {page_config['website']} (Sheet: {loader.sheet_name}).")
...
def schedule_all_jobs(config, loader, run_once=False):
    ...
    for page in pages:
        ...
        print(f"Scheduled {page.get('website','site')} for {page.get('url','url')} every {page['timeframe']} ({tf_seconds} sec).")
    if not run_once:
        scheduler.start()
...
def main():
    ...
    if not creds or not sheet_name:
        print("Google Sheets credentials_json and sheet_name must be provided in config.json under google_sheet.")
        return
